...
5b. **Entry FOK retry escalation** — FOK rejections on entry no longer trigger a cooldown. Instead, each rejection increments a per-market retry counter and adds `entry_slippage_retry_step` to the next attempt's slippage (attempt 1: base, attempt 2: +step, attempt 3: +2×step), capped at `entry_slippage_max`. After 3 attempts the counter resets and the cycle repeats. Signal evaluation remains the only gate — if momentum fades below entry_threshold, retries stop naturally. Motivation: strong OFI signals were being blocked by thin Polymarket books during fast BTC moves. The old 30-second post-FOK cooldown was locking the bot out of the entire move. Retrying with escalating slippage catches fills as market makers re-post asks after repricing.
...
10. **Config/signal logging** — Every trade logs `config_snapshot` (all thresholds, weights, persistence settings) and `signal_data` (momentum, OFI, VWAP drift, intensity, prices, seconds remaining, bias_direction, bias_adjustment) as JSONB columns for backtesting. `bias_direction` is FAVORABLE (bias lowered threshold, trading with macro trend), UNFAVORABLE (raised threshold, against trend), or NEUTRAL (no bias active).
...
- `trailing_stop_pct` (float, default 0.12) — exit when price drops X% from HWM. **Breakeven protection**: floor never drops below `entry_price`. FOK is placed at `floor - exit_slippage`, so the limit goes in slightly below entry but fills at market bid which is typically at or above entry. Previously used `entry_price + exit_slippage` as the floor, which made the stop fire $0.05 too early and rendered `trailing_stop_pct` irrelevant for any HWM < entry/(1-pct). Log shows "breakeven protect" when breakeven floor is binding vs "drawdown X%" when trailing pct is binding.
...
- `entry_slippage` (float, default 0.02) — 2 cents above market for first FOK entry attempt. Currently set to 0.04 in DB.
- `entry_slippage_retry_step` (float, default 0.02) — added to slippage on each FOK retry (attempt 2: +0.02, attempt 3: +0.04). Set to 0 to disable escalation.
- `entry_slippage_max` (float, default 0.10) — hard cap on entry slippage across all retries.
- `exit_slippage` (float, default 0.05) — 5 cents below market floor for FOK. Was 0.02, caused repeated FOK rejections
- `trade_cooldown` (float, default 30.0) — 30 seconds between trades on same market. Was 10 — too fast, caused buy-sell-rebuy at worse price. FOK rejections do NOT trigger this cooldown — signal is the only gate on retries.
...
- `trend_bias_strong_pct` (float, default 0.003) — 0.30% move in the last 5 minutes = strong trend, hard blocks counter-trend entries. Calibrated for actual 5-minute moves — the DB fallback now correctly uses a 5-minute price slice so this threshold is applied consistently.
...
- `adaptive_bias_enabled` (bool, default true) — shifts entry thresholds per-side based on 30m macro trend
...
- `low_vol_max_price_change` (float, default 0.0005) — max abs price change (fractional) to be "low vol". Uses `flow_30s.vwap_drift` (recent 30s price activity) — NOT cumulative window move (`price_change_pct`). The cumulative move stays large after an early-window spike even when the market is currently dead flat.
...
- `acceleration_tolerance` (float, default 0.10) — how much momentum can fade between ticks and still pass the acceleration filter. 0.05 was too strict — normal signal variance would reset the persistence timer repeatedly, preventing entries on legitimate sustained signals. 0.10 gives room to breathe while still blocking genuine rollover. 0.15 = loose (allows signals that plateau).
...
**Current status: `poly_book_enabled = false` (deferred).** The infrastructure is fully built and ready. Deferral rationale: (1) the chop-aware exit fix addresses the premature-exit root cause more cleanly and needs a few days of data to evaluate in isolation — can't attribute results if both ship simultaneously; (2) the imbalance features (entry veto, exit override imbalance check) are susceptible to bot order skew (e.g. 1-cent bids stacking around $0.50 creating fake directional signal). **Plan to enable**: after chop fix has been evaluated, enable with depth-only settings that bypass imbalance entirely — set `poly_book_imbalance_weight=0.0`, `poly_book_imbalance_veto=-1.0` (effectively disabled), `poly_book_exit_override_imbalance=0.0` (depth-only exit override). This uses bid depth as a pure liquidity check without relying on imbalance metrics that bots can skew.

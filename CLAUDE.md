...
- `trailing_stop_pct` (float, default 0.18) — exit when price drops X% from HWM. Pure HWM-based floor — no breakeven protection. Breakeven floor was removed because it ejected positions at entry price on normal dips before max_loss_pct could apply. `max_loss_pct` is now the sole downside protection below entry. Log shows "drawdown X% ≥ Y%" when stop fires.

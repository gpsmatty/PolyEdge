...
- `trailing_stop_pct` (float, default 0.12) — exit when price drops 12% from HWM. Was 0.25 (too loose, exited below entry). **Breakeven protection**: once armed, floor never drops below entry price — prevents winners turning into losers when HWM is modest (e.g. entry $0.53, HWM $0.64, 18% stop would floor at $0.525 without protection). Log shows "breakeven protect" when this kicks in vs normal drawdown trigger. EXPERIMENTAL — monitor whether this reduces trailing stop winner P&L by exiting too early on dips that would have recovered.

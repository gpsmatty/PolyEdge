...
- `trailing_stop_pct` (float, default 0.12) — exit when price drops 12% from HWM. Was 0.25 (too loose, exited below entry). **Breakeven protection**: once armed, floor never drops below `entry_price + exit_slippage` — ensures the FOK sell fills at or above entry (breakeven). Without this fix, the floor was set to bare `entry_price`, but the FOK placed at `floor - exit_slippage` would fill at `entry_price - exit_slippage` = guaranteed small loss. Log shows "breakeven protect" when this kicks in vs normal drawdown trigger.
